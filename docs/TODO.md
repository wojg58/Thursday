...
- [x] `lib/utils/map-utils.ts` - 좌표 변환 유틸리티
- [x] `components/home-layout.tsx` - 리스트+지도 레이아웃 컴포넌트
- [x] 관광지 마커 표시
- [x] 마커 클릭 시 인포윈도우
- [x] 리스트-지도 연동 (클릭)
...
- [x] `app/places/[contentId]/page.tsx` - 페이지 생성
...
- [x] `components/tour-detail/detail-map.tsx` - 상세페이지 지도 컴포넌트
